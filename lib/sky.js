		...
		if ( Meteor.isServer ) {
			...
			if ( video ) {
				...
				if ( video.type == 'track' ) {
					...
					var moreVideos = Meteor.http.get('https://www.googleapis.com/youtube/v3/search?part=id&relatedToVideoId='+video.youtube_id+'&type=video&key=AIzaSyCjkQ_YauVPcAHM541qjYVtWOX7kjYFSlE&maxResults=15').data;
					...
					randomVideos.forEach(function(item, i){
						if ( recommendationsCount == numberOfRecommendations ) {
							...
							var videoInfo = Meteor.http.get('https://www.googleapis.com/youtube/v3/videos?part=id%2Csnippet%2CcontentDetails%2CtopicDetails&id='+item.id.videoId+'&maxResults=1&key=AIzaSyCjkQ_YauVPcAHM541qjYVtWOX7kjYFSlE');
